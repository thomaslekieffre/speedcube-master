...
const DEFAULT_SCRAMBLE_LENGTHS: Record<string, number> = {
  ...
  '555': 60,     // 60 mouvements
  '666': 80,     // 80 mouvements
  '777': 100,    // 100 mouvements
  'clock': 0,    // 9 mouvements par défaut
  'minx': 70,    // 70 mouvements
  'pyram': 10,   // 10 mouvements
  'skewb': 0,    // 7 mouvements par défaut
...
export function generateScramble(puzzleType: string): string {
  try {
    ...
    if (!scrambleType) {
      console.warn(`Type de puzzle non supporté: ${puzzleType}, utilisation du 3x3x3`);
...
export function generateScrambleImage(scramble: string, puzzleType: string): string {
  try {
    const scrambleType = PUZZLE_SCRAMBLE_TYPES[puzzleType];
